...
func handleRequestOPTIONS(cfg config.Config, w http.ResponseWriter, r *http.Request) {
	slog.Debug("Handling OPTIONS request", "url_path", r.URL.Path)
	...
	proxyReq, err := http.NewRequest(http.MethodOptions, target, nil)
	if err != nil {
	...
	slog.Debug("Done handling the OPTIONS request")
}

func handleRequestGET(cfg config.Config, w http.ResponseWriter, r *http.Request) {
	slog.Debug("Handling GET request", "url_path", r.URL.Path)
	...
	slog.Debug("Done handling the GET request")
}
